...
- 未完成事项：下一步需要在不降低默认多核调度的前提下继续降低对象并行阶段峰值；优先考虑减少对象 worker 并发时的 retained object graph 或在对象阶段结束后触发可证明有效的受控压缩/收集，而不是仅缩小扫描结果窗口。最终候选 RSS 必须不高于 6,957,772 KiB；随后跑同提交 `--workers 1` 对照、container-only 和全量 Convert，分别核对多核调度、10,197,212 KiB 与 11,474,792 KiB 门槛。
