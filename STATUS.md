...
- 未完成事项：提交并推送对象范围修正版，在 Debian 13 以固定输入确认默认 4 worker 的多核 CPU、wall、RSS、输出与清理；通过后重跑 container-only 和全量 Convert 内存门禁及 Convert 导出 CPU 采样；全部通过后关闭 Phase 2。
