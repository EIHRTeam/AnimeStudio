...
- 未完成事项：容器块候选 `b59ba49f7c8f05e1b3755527a8a283643ad5e603` 已推送并由 Debian 从 GitHub 精确检出，Release 构建、Core smoke、Linux 动态 package/runtime smoke 通过，归档 SHA256 为 `6f60cb4d0976ec69824f0deee0231d9dab1e7306c9255fb73fd1f0f3c9a5ec01`。但固定 `.chk` 的实时采样显示它由大量单块 VFS 串接而成，前 41 秒仍只有主线程、平均 103.463% CPU、无 `AnimeStudio container` 线程；该诊断运行已主动 `SIGTERM`，非 OOM，不作为性能结果。范围候选 `4358864545b81eadfe103489f38faf0ee7eeebc4` 已推送并由 Debian 精确检出，Release/Core/Linux package runtime smoke 通过，归档 SHA256 为 `2a532ce6d4a0f68e4a4993cc9841725266f6270db78a941d3ba2a5872d256246`。75 秒快速审计无解析错误，平均 289.5%、峰值 368% CPU，63/75 个样本超过 100%，三个 `AnimeStudio container` 线程持续分布于不同 CPU。随后完整运行在 4:38 进入对象扫描时暴露 `PPtr.TryGetAssetsFile` 对普通 `Dictionary` 的并发 `TryGetValue`/`Add` 竞争，同一 CAB 键重复插入；已主动 `SIGTERM`，非 OOM，不作为兼容或性能结果。停止前平均 CPU 271.106%、峰值 370%、采样峰值 RSS 7,079,708 KiB、临时文件 5,336,646,616 字节。下一步将文件索引缓存改成原子 `ConcurrentDictionary.GetOrAdd`，并根据最终成功运行的 RSS 决定是否收紧 512 MiB 范围并发预算；之后重新完整验证 Loading、对象扫描、wall、RSS、输出与清理，再跑最终 `--workers 1` 对照、container-only 和全量 Convert。
