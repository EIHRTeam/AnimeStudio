...
- 未完成事项：提交并推送批次扫描候选，由 Debian 从 GitHub 精确检出后重新完成 Release/Core/Linux 动态 package smoke 和固定 AssetMap 全量回归；最终候选 RSS 必须不高于 6,957,772 KiB。随后跑同提交 `--workers 1` 对照、container-only 和全量 Convert，分别核对多核调度、10,197,212 KiB 与 11,474,792 KiB 门槛。
