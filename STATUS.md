...
- 问题与决策：并行度默认不得为 1；单核主机仍可正确运行，但不为其牺牲多核默认值。严禁多个 worker 共享可变 reader 位置；同一 `SerializedFile` 仅在文件句柄、可公开内存段或引用计数容器切片能创建独立对象范围时才对象级并行，否则受控回退串行。共享资源流必须串行 seek/read，读取完成后的转换可并行。FBX 原生库会切换进程当前目录且尚未证明可重入，因此暂不并行；其资产保持原序直到完整导出结束。`PERF_NEXT_STEP.md` 是历史规划输入，其中暂缓 Phase 2 并行化的建议已被本次用户明确要求覆盖。Server GC 已由实机数据否决，不再作为 Phase 2 条件。
- 未完成事项：提交并推送容器块并行候选，在 Debian 13 从 GitHub 检出精确提交后验证 Loading 命名线程、多核 CPU、wall、RSS、输出与清理；通过后以最终代码分别跑固定 `--workers 1`/`--workers 4` 对照，并重跑 container-only 和全量 Convert 内存门禁及 Convert 导出 CPU 采样；全部通过后关闭 Phase 2。
