...
- 未完成事项：容器块候选 `b59ba49f7c8f05e1b3755527a8a283643ad5e603` 已推送并由 Debian 从 GitHub 精确检出，Release 构建、Core smoke、Linux 动态 package/runtime smoke 通过，归档 SHA256 为 `6f60cb4d0976ec69824f0deee0231d9dab1e7306c9255fb73fd1f0f3c9a5ec01`。但固定 `.chk` 的实时采样显示它由大量单块 VFS 串接而成，前 41 秒仍只有主线程、平均 103.463% CPU、无 `AnimeStudio container` 线程；该诊断运行已主动 `SIGTERM`，非 OOM，不作为性能结果。当前范围并行修正在本地通过 Release 构建、Core smoke、macOS ARM64 动态 package/runtime smoke、Linux x64 与 Windows x64 跨平台 package smoke及 `git diff --check`；新增 smoke 验证四个串接 UnityFS 范围的精确 offset/length、独立视图和加密布局回退。下一步提交并推送该候选，在 Debian 做快速线程审计后完整验证 Loading 命名线程、多核 CPU、wall、RSS、输出与清理；通过后以最终代码分别跑固定 `--workers 1`/`--workers 4` 对照，并重跑 container-only 和全量 Convert 内存门禁及 Convert 导出 CPU 采样；全部通过后关闭 Phase 2。
