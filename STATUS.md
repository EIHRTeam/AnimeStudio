...
- 后续实测 4：精确提交 `a702711ed179d029fdfcbcb11c18cc798a925d03` 已推送并由 Debian 精确检出，Release/Core/Linux 动态 package runtime smoke 通过，Linux 归档 SHA256 `83674d93ad51132c82e6ef3ccff5f7c0375e640f11d1ce74aec49f51fcbaf416`。固定 AssetMap `--workers 4` 在输出 writer 开始前采到 RSS 6,992,736 KiB，仍高于 6,957,772 KiB 门槛；该 run 当场终止，未继续跑全量回归。1,024 批次窗口没有继续降低峰值，说明主要余量不再由单批 `AssetMapObjectScan` 数组决定；下一候选改为 256 批次并复核 `matches`/PPtr 关系保留图是否还能拆分。
- 当前工作树补充 4：准备继续收紧对象扫描批次窗口到 256，保留 4 核下 2 个高保留 worker、容器多核调度和加载后 compacting GC 不变；目标是确认剩余峰值是否来自批内对象关系保留。如果 256 仍无法低于基线，将改为减少对象扫描结果中非必要关系的跨阶段保留，而不是把默认并行度退回 1。本地 `dotnet build AnimeStudio.sln -c Release`、Core smoke、三 RID package smoke、`git diff --check` 和活跃项目旧 TFM/桌面引用扫描均通过；源码扫描仅命中 `scripts/package-deb.sh` 中合法的 ELF AppHost 打包注释。仅有既有 `MessagePack` NU1903、nullable 和 TODO 警告。
- 后续实测 5：精确提交 `656d042873a3684f3ad85c8e1c428cf02d030d37` 已推送并由 Debian 精确检出，Release/Core/Linux 动态 package runtime smoke 通过，Linux 归档 SHA256 `b020e238f62322e7ce7206cbe77bfa7bc28bcbf8096ffdc75701d3b2663dc65c`。固定 AssetMap `--workers 4` 在输出 writer 开始前采到 RSS 6,968,712 KiB，仍高于 6,957,772 KiB 门槛；该 run 当场终止，未继续跑全量回归。256 批次有效但只剩约 11 MiB 超线，下一候选改为去掉对象扫描阶段对完整解析对象的非必要长期保留：容器和 MiHoYo `IndexObject` 解析直接用 PPtr 目标 `(SerializedFile, PathID)` 更新 asset record，只保留 Animator 命名所需的 `GameObject` 到 `ObjectsDic`。
- 当前工作树补充 5：准备让 AssetMap PPtr 解析不再要求为每个对象保留 `Object` 实例。`PPtr` 将暴露内部目标引用解析，AssetMap 用轻量 `(SerializedFile, PathID)` key 查找条目；`SerializedFile.ObjectsDic` 只缓存 Animator 命名需要的 `GameObject`，避免把所有扫描过的基础 `Object` 都保留到文件结束。Core smoke 同步改为验证 TextAsset 批量扫描不会保留非 PPtr 目标对象。本地 `dotnet build AnimeStudio.sln -c Release`、Core smoke、三 RID package smoke、`git diff --check` 和活跃源码旧 TFM/桌面引用扫描均通过；仅有既有 `MessagePack` NU1903、nullable 和 TODO 警告。
...
- 本次目标：引入用户级性能配置 `~/.anime/config.json` 与 `--mode default|limit|fast`，把性能指标定义为「在预算内尽可能优化占用、加快速度」的优化预算而非硬节流；按 `PLAN_PERFORMANCE_PROFILE.md` 分阶段实现并同步本地各类文档。背景：上一轮调查确认 Debian 上 AssetMap 仅用到 ~65% CPU/~55% 内存的根因是 `WorkerBudget` 为守 6,957,772 KiB 门槛把 `--workers 4` 砍半成 2。
- 完成内容：阶段 1（配置模型与加载器，零行为变化）、阶段 2（模式感知 `WorkerBudget` + resolver，仍不接线）、阶段 3（CLI 接线激活）、阶段 4（文档全同步）。`fast`/`limit` 解除 worker 砍半并按预算填满并发，`default` 保留砍半与既有 RSS 门槛；RAM 为软预算（推导 worker 数与容器内存阈值），不改 GC；CPU 仅控 worker 数；`--mode` 由 CLI 覆盖 config，显式 `--workers` 优先级最高。
- 修改文件/接口：新增 `AnimeStudio.CLI/PerformanceProfile.cs`（`PerformanceMode` 枚举 + `PerformanceConfig`/`AdvancedOverrides` POCO + `Load()`，发现 `~/.anime/config.json`，支持 `ANIMESTUDIO_CONFIG_PATH` 覆盖）、`AnimeStudio.CLI/PerformanceResolver.cs`（`ResolvedPerformance` + 优先级链与软预算公式，每-worker≈1 GiB 默认）、仓库根 `PLAN_PERFORMANCE_PROFILE.md`；`WorkerBudget` 改为可配置静态砍半策略并新增 `ConfigureHalving`（默认 `true` 逐字保留旧行为）；`AssetsHelper` 新增 `SetParseWorkerHalving`；`Settings.SerializerOptions` 改 `internal` 并新增 `GetContainerStorageOptions(long?)` 重载；`CommandLine` 新增 `Option<PerformanceMode?> Mode` 与 `WorkersExplicitlySet`（`GetResult(Workers) is { Tokens.Count: > 0 }`）；`Program.Run` 接线 load+resolve+apply 到两个 `AssetsManager` 实例、线程池、容器选项、`WorkerBudget` 策略与导出路径，新增 `Performance mode:` 日志；Core/CLI smoke 新增断言。文档：追加式更新 `CLAUDE.md`、`ROADMAP.md`、`PLAN.md`、`PERF_NEXT_STEP.md`、`PERF_ANALYZE_REPO.md`。
- 验证及指标：本地 `dotnet build AnimeStudio.sln -c Release` 0 error（仅既有 MessagePack NU1903、nullable、TODO 警告）；Core.Smoke 通过（含新增 `VerifyWorkerBudgetHalvingToggle`：禁用砍半时 `8→8/4→4/2→2/1→1`、恢复后回 `8→4`、`0` 仍抛、finally 复位）；osx-arm64 `dotnet publish` + CLI.Smoke 通过（含新增 `VerifyPerformanceConfigLoad`：解析/大小写不敏感/损坏 JSON 容错/缺文件；`VerifyPerformanceModeWiring`：fast halving-off、CLI `--mode` 覆盖 config、显式 `--workers` 优先、向后兼容基线、极小 RAM 预算（`limit` + `maxMemoryKB:1`）把有效 worker 钳到 1）。另注：请求 worker 数为 0 时抛 `ArgumentOutOfRangeException` 由 Core.Smoke 的 `VerifyWorkerBudgetHalvingToggle`/`VerifyMemoryStableWorkerBudget` 断言，与 resolver 的 RAM 预算钳位是两回事。期间发现并修复新增重载导致 `VerifyStreamingConfiguration` 的 `GetMethod` 歧义（改用 `Type.EmptyTypes` 选无参重载）。未跑三 RID package smoke 与 Debian 实机（见未完成）。
- 问题与决策：性能指标定义为优化预算而非节流；`default` 保守不变以守门槛（无配置无标志逐项=今天），`limit`/`fast` 解除砍半填预算。RAM 走软预算、明确不动 GC（`HeapHardLimitPercent=75` 仍是 RSS 硬天花板，Server GC 仍被否决），`fast` 可超历史门槛但以本机 15 GiB RAM 或 `maxMemoryKB` 为界。`--mode` 用 `Option<PerformanceMode?>`（未给为 null，区分显式 `--mode default`）。**经用户在本会话显式授权，对平时受「不得修改」规则保护的未跟踪 `PERF_ANALYZE_REPO.md` 做了追加式最小改动（仅加「落地状态」一节，不重写分析与倍率）**；该授权为本会话例外，不作为发布溯源。开放点：`default` 是否也转向优化（会触及 Phase 2 RSS 门槛），当前推荐保持保守，待用户确认。
- 未完成事项：三 RID package smoke（仅跑了 osx-arm64）；提交/推送（按规则待用户明确要求后进行，并据此回填起止提交 hash）；Debian `1.14.226.195` 实机对比 `--mode default`（应仍 ~50% CPU、RSS 守门槛）与 `--mode fast`（应 CPU 显著>1 核、RSS 在预算内），授权后记录指标。
- 后续注意事项：性能模式不得改变 AssetMap JSON/MessagePack 字节与 XML 规范化结果；`fast` 仍受 75% 堆硬上限约束，非「无上限」；两个 `AssetsManager` 实例的 WorkerCount/Container 配置须保持同步（已同步），`WorkerBudget` 砍半为进程级静态天然一致；`BundleFile.cs:134/145` 等公开/2-arg 容器构造函数仍硬编码 `Environment.ProcessorCount`，不在 CLI 主提取路径，本次未改，列为已知边界；旧有「不得修改 `PERF_ANALYZE_REPO.md`」措辞之外的本次授权例外已在此记录。
- 本次补充（提交前对抗式预检 + 修正）：提交前用多 agent 对抗式审查工作流（10 agent，5 维度审查 + 逐项核实）复查 diff，确认 5 个 low 级问题并全部修复——(1)(2) `default` 模式改为完全忽略 config 的优化旋钮（`containerMemoryThresholdMiB`/`advanced.minimumWorkerThreads`/`advanced.halveParseWorkers`），保证有无 config 都逐项=今天且守门槛，优化旋钮仅 `limit`/`fast` 生效；(3) `PerformanceConfig.Load` 把 `GetConfigPath()`/`Path.GetFullPath` 纳入 try 并扩大 catch（增 `ArgumentException`/`NotSupportedException`/`SecurityException`），畸形 `ANIMESTUDIO_CONFIG_PATH` 优雅退化为默认而非中止运行；(4)(5) 修正 STATUS/`PLAN_PERFORMANCE_PROFILE.md` 文案（worker 钳位措辞、limit 两个 lever 解耦）。修复后本地 `dotnet build -c Release` 0 error、Core.Smoke、三 RID package smoke（osx-arm64 runtime + linux-x64/win-x64 跨平台）、`git diff --check` 均通过。
- 实机验证（Debian 13 / 4 核 / 15 GiB，2026-06-18；以 u202f 经 GitHub 检出 `76b5f7d`，在服务器 `dotnet publish` linux-x64 构建，原生库齐全、runtimeconfig 未变）：固定基线文件 `VFS/7064D8E2/68B3B9B8EB82E88FBFE6A313E6B18FB6.chk`（1,812,594,931 字节，283,596 资产）AssetMap `--map_type XML` 对比——`--mode default`：4 workers/砍半 on、CPU 131%、wall 10:05.22、峰值 RSS 6,811,548 KiB、Loading 340,096.604 ms/Object scanning 247,383.824 ms；`--mode fast`：4 workers/砍半 off、CPU 204%、wall 7:41.56、峰值 RSS 6,896,620 KiB、Loading 240,887.689 ms/Object scanning 203,215.303 ms。fast 相对 default CPU +73 个百分点、墙钟快约 24%、加载与对象扫描两阶段均更快；二者峰值 RSS 均低于历史门槛 6,957,772 KiB（受益于既有对象保留削减），GC 配置未变（Server=false/HeapHardLimitPercent=75/RetainVM），临时目录清空，`oom_kill` 0。输出正确性：两模式 XML 在规范化易变 `filename`/`createdAt` 后 SHA256 完全一致（`c563e85f09aaa97aa3cece271ceb224359c04a429f95655fcbb461f23e9870e2`），资产数同为 283,596——性能模式不改变 AssetMap 输出。说明：单个 VFS 容器下串行 Loading 段仍限制 CPU 上限（与根因分析一致），多文件输入下 fast 预期可进一步扩展。服务器构建/数据均在 u202f 家目录，未传输本机可执行文件。
- 实机 dump 吞吐验证（2026-06-19，u202f，服务器从 `8546725` 构建，`--mode fast` 全 VFS 包导出到 `~/dump-test`，600s 硬停）：扫描 792 文件 / 767 chk，按路径长度顺序处理；600s 内仍停在第 1 个 VFS 容器（其含 5459 可导出资产，导出至约 1246/5459，无 chk 完整完成），输出 1240 文件 / 6.2 MiB（3,779,176 字节），类型 MonoBehaviour/Sprite/Material/Mesh/Texture2D；峰值 RSS 1,174,060 KiB（~1.12 GiB），`oom_kill` 0，干净退出。结论：全量 dump 的瓶颈是**逐资产 Convert 转换吞吐**（非加载/内存）；性能模式的砍半开关只作用于加载/扫描阶段，导出阶段 `Studio.ExportAssets` 的 `Parallel.For` 本就用满 worker、与模式无关，故 fast 对纯导出吞吐帮助有限。下一步：在服务器 profiling 研究并优化逐资产转换时间开销（已获授权）。
- 起止提交：`2a6b177` -> `c43f981`（feat: 性能配置 + 模式）；`76b5f7d`（会话文档）；`8546725`（AssetMap 实机对比）；dump 吞吐验证记录见后续文档提交。
